...
def main():
    ...
    print("[+] Playing Games with Reference NN Model \t>", end='')
    ref_worst, ref_best, ref_avg, ref_sum = play_game(ml_model=ml_model, games=100)
    print(f"{ref_worst};{ref_avg};{ref_best};{ref_sum}")
    ...
    for current_generation in range(MAX_GENERATIONS):
        print(f"[+] Generation {current_generation+1} of {MAX_GENERATIONS} \t> ", end='')
        i = 0
        ...
        for individual in population:
            ...
            # Play the Games
            score_worst, score_best, score_avg, score_sum = play_game(ml_model=individual.ml_model, games=100, model_name=individual.specie)
        ...
        print(f"{gen_score_min};{gen_score_avg};{gen_score_max};{gen_score_sum}")

        # Evolve to Next Population
